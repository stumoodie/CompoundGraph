...
//			exactly(1).of(destnrootChildEdgeFactory).createEdge(); will(returnValue(mockEdge));
			...
			exactly(5
					).of(mockElementFactory).createNode(); will(returnValue(mockNode));
			...
			one(destnFixture.getGraph()).notifyGraphStructureChange(with(any(IGraphStructureChangeAction.class)));
		}});
